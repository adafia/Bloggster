...
export default class Upload extends Component {
  ...
  async uploadMultipartFile() {
    try {
      ...
      console.log('<<<:::::::>>>', completeUploadResp.data.data.Location);
      this.setState({ location: completeUploadResp.data.data.Location });
    } catch (err) {
      console.log(err);
    }
  }
      ...
      <div>
        ...
        {!this.state.location ? (
          ...
          <video height='540' controls>
            <source src={this.state.location} type='video/mp4' />
          </video>
        )}
      </div>
    );
